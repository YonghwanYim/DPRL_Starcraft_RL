...
class Agent(base_agent.BaseAgent):
    def __init__(self, env):
        ...
        # QNet = MLP_QNet
        QNet = DDQN
        self.qnet_A = QNet(self.dimState, self.dimAction, self.hidden_dim)
        self.qnet_B = QNet(self.dimState, self.dimAction, self.hidden_dim)
        ...
        self.optimizer_A = Adam(self.qnet_A.parameters(), lr=self.lr, weight_decay=0)
        self.optimizer_B = Adam(self.qnet_B.parameters(), lr=self.lr, weight_decay=0)
    ...
    def train(self):
        ...
        out = self.qnet_A.forward(s)
        pred = out.gather(1, a)
        out_tar = self.qnet_B.forward(sp)
        target = r.unsqueeze(1) + self.gamma * out_tar.gather(1,self.qnet_A.forward(sp).max(1)[1].unsqueeze(1)) * (1 - done.unsqueeze(1))
        loss = F.mse_loss(pred, target)
        loss.backward()
        torch.nn.utils.clip_grad_norm_(self.qnet_A.parameters(), MAX_NORM) # gradient clipping
    ...
    def runEpisode(self, test=False):
        ...
        while True:
            ...
            if test:
                a = self.qnet_A(s_tensor).argmax().item()
            else:
...
def main(args):
    #trainAgentDQN('DDQN.pt')
    #trainContinuingAgentDQN('DDQN.pt')
    #testBestAgentDQN('DDQN.pt')
    testManyAgentDQN('DDQN.pt')
